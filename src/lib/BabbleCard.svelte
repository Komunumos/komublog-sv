<script lang="ts">
  ...
  import getAvatarPath from './avatarHelper';
  ...
  <article class="card" data-theme="light">
    <div class="header">
      {#if babble.avatar}
        <img src="{getAvatarPath(babble.author, babble.avatar)}" alt="Author Avatar" />
      {:else}
    ...
    <div class="actions">
      <button class="outline" on:click={like}>
        <span role="img" aria-label="Like">❤️</span> {babble.likes}
      </button>
      <button class="outline" on:click={retweet}>
        <span role="img" aria-label="Retweet">🔄</span> {babble.reblabs}
      </button>
